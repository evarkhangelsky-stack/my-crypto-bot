...
class TradingSessions:
    ...
    SESSIONS = {
        'asia': {
            ...
            'strategy': 'range',
            'color': '🟡',
            'trade_multiplier': 0.9  # Увеличено с 0.8
        },
        'london': {
            'name': '🇬🇧 Лондонская',
            'start': 8,
        ...
        'sunday_open': {
            ...
            'color': '🌟',
            'trade_multiplier': 1.0  # Увеличено с 0.9
        }
    }
...
class BybitScalpingBot:
    def __init__(self):
        ...
        self.symbol_config = {
            ...
            'SOL/USDT:USDT': {
                'risk_pct': 0.004,  # Увеличено с 0.003
                'min_size': 0.1,
                'leverage': 5,
                'volatility_factor': 0.9,  # Увеличено с 0.8
                'max_positions_per_day': 4
            },
            'XRP/USDT:USDT': {
                'risk_pct': 0.005,  # Увеличено с 0.004
                'min_size': 10,
                'leverage': 5,
                'volatility_factor': 1.0,  # Увеличено с 0.9
                'max_positions_per_day': 4
            }
        }
        ...
        self.positions_file = "positions_backup.json"

        # Общие настройки
        self.sl_atr_multiplier = 1.5  # Увеличено с 1.2 для более широких стопов
        self.tp_atr_multiplier = 2.5  # Увеличено с 2.0 для лучшего риск/прибыль
        self.trailing_stop_percent = 0.5
        self.taker_fee = 0.0006

        self.max_hold_time = timedelta(hours=3)  # Увеличено с 2 часов
        self.min_profit_for_breakeven = 0.5  # Увеличено с 0.3
        self.trailing_activation = 0.8  # Увеличено с 0.5
        self.trailing_distance = 0.4  # Увеличено с 0.3
        self.min_balance_for_trading = 50

        # Лимиты
        self.daily_loss_limit_pct = -5.0  # Увеличено с -4.2 (больше риска)
        ...
        # Загружаем сохраненные позиции (на случай перезапуска)
        self.load_positions()
        ...
        print(f"[{datetime.now(timezone.utc)}] SL/TP: {self.sl_atr_multiplier}x/{self.tp_atr_multiplier}x ATR")
        self.send_telegram(f"🤖 *Bot restarted - REAL TRADING*\nSymbols: {', '.join(self.symbols)}\nTimeframe: {self.timeframe}\nMax positions: {self.max_concurrent_positions}\nSL/TP: {self.sl_atr_multiplier}x/{self.tp_atr_multiplier}x ATR")

    def save_positions(self):
        """Сохраняет открытые позиции в файл"""
        positions_data = {}
        for symbol, pos in self.positions.items():
            if pos:
                pos_copy = pos.copy()
                # Конвертируем datetime в строку
                if 'open_time' in pos_copy:
                    pos_copy['open_time'] = pos_copy['open_time'].isoformat()
                positions_data[symbol] = pos_copy
        
        with open(self.positions_file, 'w') as f:
            json.dump(positions_data, f, indent=2)
        print(f"💾 Позиции сохранены: {len(positions_data)} активных")
        ...
        try:
            ...
            for symbol, pos_data in positions_data.items():
                if symbol in self.positions:
                    ...
                    if 'open_time' in pos_data:
                        pos_data['open_time'] = datetime.fromisoformat(pos_data['open_time'])
                    self.positions[symbol] = pos_data
                    print(f"📂 Загружена позиция: {symbol} {pos_data['side']} от {pos_data['open_time']}")
            if positions_data:
                print(f"📂 Загружено {len(positions_data)} позиций")
        except FileNotFoundError:
            print("📂 Нет сохраненных позиций")
        except Exception as e:
    ...
    def _calculate_signal_strength(self, df, signal, orderbook):
        ...
        if signal == 'LONG':
            if 40 < last['rsi'] < 70:  # Расширенный диапазон
                strength += 0.1
            if last['ema_20'] > last['ema_50']:
                strength += 0.1
            if last['close'] > last['vwap']:
                strength += 0.1
            if orderbook['bid_ratio'] > 52:  # Понижен порог
                strength += 0.1
            if last['macd_hist'] > -0.5:  # Мягче условие
                strength += 0.1
        else:
            if 30 < last['rsi'] < 60:  # Расширенный диапазон
                strength += 0.1
            if last['ema_20'] < last['ema_50']:
                strength += 0.1
            if last['close'] < last['vwap']:
                strength += 0.1
            if orderbook['bid_ratio'] < 48:  # Понижен порог
                strength += 0.1
            if last['macd_hist'] < 0.5:  # Мягче условие
                strength += 0.1
        
        if last['rsi'] > 80 or last['rsi'] < 20:  # Расширены границы экстремумов
            strength -= 0.2
        if last['adx'] < 20:
            strength -= 0.1
        if last['adx'] > 35:  # Понижен порог
            strength += 0.1
...
Should we take this scalp trade? Reply YES or NO."""
            ...
            res = requests.post(
                ...
                json={
                    ...
                    'temperature': 0.8,  # Еще выше для разнообразия
                    'max_tokens': 10
                },
                timeout=15
            ).json()
            ...
            # УВЕЛИЧЕНО ДО 60% случайный фактор для отклоненных сигналов
            if not positive and np.random.random() < 0.6:
                self.ai_stats['random_taken'] += 1
                print(f"🎲 Случайный фактор (60%): берем сделку несмотря на NO")
                return True
    ...
    def oversold_strategy(self, df, ob):
        """Стратегия для покупки на перепроданном рынке"""
        last = df.iloc[-1]
        price = last['close']
        rsi = last['rsi']
        ema20 = last['ema_20']
        ...
        # LONG при перепроданности - более мягкие условия
        if rsi < 40 and price < ema20 * 1.01 and bid_ratio > 45:  # Увеличено RSI до 40
            if rsi < 30:
                strength = 0.7
                print(f"📊 Oversold: RSI {rsi:.1f} - сильная перепроданность")
            elif rsi < 35:
                strength = 0.6
                print(f"📊 Oversold: RSI {rsi:.1f} - средняя перепроданность")
            else:
                strength = 0.5
                print(f"📊 Oversold: RSI {rsi:.1f} - легкая перепроданность")
            
            # Дополнительный буст если цена у поддержки
            if price < ema50 * 0.99:
                strength += 0.1
                print(f"📊 Oversold: цена ниже EMA50, дополнительный буст +0.1")
                
            return 'LONG', strength
        
        return None, 0

    def overbought_strategy(self, df, ob):
        """Стратегия для продажи на перекупленном рынке"""
        last = df.iloc[-1]
        price = last['close']
        rsi = last['rsi']
        ema20 = last['ema_20']
        ...
        # SHORT при перекупленности
        if rsi > 60 and price > ema20 * 0.99 and bid_ratio < 55:  # Новые условия
            if rsi > 70:
                strength = 0.7
                print(f"📊 Overbought: RSI {rsi:.1f} - сильная перекупленность")
            elif rsi > 65:
                strength = 0.6
                print(f"📊 Overbought: RSI {rsi:.1f} - средняя перекупленность")
            else:
                strength = 0.5
                print(f"📊 Overbought: RSI {rsi:.1f} - легкая перекупленность")
            
            # Дополнительный буст если цена выше EMA50
            if price > ema50 * 1.01:
                strength += 0.1
                print(f"📊 Overbought: цена выше EMA50, дополнительный буст +0.1")
    ...
    def sideways_strategy(self, df, ob):
        ...
        if price <= bb_lower and rsi < 45 and stoch_k < 35 and bid_ratio > 50:
            strength = 0.6 if rsi < 35 and stoch_k < 25 else 0.4
            return 'LONG', strength
        
        if price >= bb_upper and rsi > 55 and stoch_k > 65 and bid_ratio < 50:
    ...
    def trend_strategy(self, df, ob):
        ...
        if (price > vwap and ema20 > ema50 and 
            rsi > 40 and rsi < 75 and  # Расширен диапазон
            macd_hist > -0.5 and  # Мягче условие
            stoch_k < 85 and  # Мягче условие
            bid_ratio > 50):  # Мягче условие
            
            strength = 0.7
            if rsi > 45 and stoch_k > 45 and bid_ratio > 55:
                strength = 0.8
            return 'LONG', strength
        
        if (price < vwap and ema20 < ema50 and 
            rsi < 60 and rsi > 25 and  # Расширен диапазон
            macd_hist < 0.5 and  # Мягче условие
            stoch_k > 15 and  # Мягче условие
            bid_ratio < 50):  # Мягче условие
            
            strength = 0.7
            if rsi < 55 and stoch_k < 55 and bid_ratio < 45:
                strength = 0.8
            return 'SHORT', strength
    ...
    def basic_strategy(self, df, ob):
        ...
        if price > ema20 and rsi > 35 and rsi < 75:  # Расширен диапазон
            strength = 0.4  # Увеличена базовая сила
            return 'LONG', strength
        
        if price < ema20 and rsi < 65 and rsi > 25:  # Расширен диапазон
            strength = 0.4
            return 'SHORT', strength
    ...
    def detect_signal(self, symbol, df):
        ...
        oversold_sig, oversold_strength = self.oversold_strategy(df, ob)
        overbought_sig, overbought_strength = self.overbought_strategy(df, ob)
        ...
        signals = []
        if oversold_sig:
            signals.append(('oversold', oversold_strength, oversold_sig))
        if overbought_sig:
            signals.append(('overbought', overbought_strength, overbought_sig))
        if side_sig:
        ...
        if signals:
            # Выбираем сигнал с максимальной силой
            best_signal = max(signals, key=lambda x: x[1])
            final_signal = best_signal[2]
            final_strength = best_signal[1]
            print(f"[{now}] 📊 Выбран {best_signal[0]} сигнал (сила {best_signal[1]})")

        if not final_signal:
            # Запасной сигнал на основе EMA
            if last['close'] > last['ema_20']:
                final_signal = 'LONG'
                final_strength = 0.3
        ...
        if final_signal:
            ...
            if context['trend'] == 'BULL' and final_signal == 'LONG':
                boost = min(0.25, context['strength'] * 0.4)
                ...
                penalty = min(0.25, context['strength'] * 0.3)  # Меньше штраф
                ...
                penalty = min(0.25, context['strength'] * 0.3)
                final_strength = max(0, final_strength - penalty)
                print(f"[{now}] ⚠️ MTF штраф: -{penalty:.2f}")
        ...
        # Пониженный порог входа
        threshold = 0.18  # Уменьшен с 0.20
        ...
        if final_signal and final_strength >= threshold:
            ...
            print(f"    SL: {sl:.2f}, TP: {tp:.2f}, Комиссия учтена: {fee_adj:.2f}")
            ...
            if final_signal:
                ...
                print(f"[{now}] ❌ Нет сигналов от стратегий")
    ...
    def place_order(self, symbol, signal, params):
        try:
            ...
            msg = (
                ...
                f"Размер: {size}\n"
                f"Риск: {risk_pct*100}%\n"
                f"Плечо: {self.symbol_config[symbol]['leverage']}x\n"
    ...
    def manage_position(self, symbol, df):
        ...
        if hold_time > self.max_hold_time:
            if pnl_pct > 0:
                ...
                # Сужаем TP
                new_tp = entry * (1 + (tp/entry - 1) * 0.8)
                pos['take_profit'] = new_tp
                print(f"⏱️ Время вышло, TP сужен до {new_tp:.2f}")
            return
        
        # Breakeven при меньшей прибыли
        if pnl_pct > 0.4 and not pos.get('breakeven_activated'):  # Снижено с 0.5
            pos['stop_loss'] = entry
            pos['breakeven_activated'] = True
            print(f"🔒 Breakeven активирован для {symbol}")
        
        # Трейлинг при меньшей прибыли
        if pnl_pct > 0.6 and not pos.get('trailing_activated'):  # Снижено с 0.8
    ...
    def save_state_before_exit(self):
        """Сохраняет состояние перед выходом"""
        print("\n💾 Сохраняем состояние перед выходом...")
        self.save_positions()
        # Сохраняем статистику AI
        with open('ai_stats_backup.json', 'w') as f:
            json.dump(self.ai_stats, f)
    ...
    def run(self):
        ...
        print(f"🎯 Порог входа: 0.18")
        print(f"💰 SL/TP: {self.sl_atr_multiplier}x/{self.tp_atr_multiplier}x ATR")
        print(f"🤖 AI случайный фактор: 60%")
        print(f"{'='*50}\n")
...
def signal_handler(sig, frame):
    print("\n🛑 Получен сигнал остановки. Сохраняем состояние...")
    if 'bot' in globals():
        bot.save_state_before_exit()
    sys.exit(0)
